...
'文言の数を調べる
    ...
    For i = 1 To NPat
        ...
        For j = 1 To 16
'            If j = 4 Or j = 9 Then
                Messag(p).Patn(j) = w(j)           '予報文にかかわる種別
'            Else
'                Messag(p).Patn(j) = Trim(w(j))     '予報文にかかわる種別
'            End If
        Next j
        m = "新川　" & Messag(p).Patn(3)
        Messag(p).Patn(3) = m
        ' 1=パターン番号            2=①予報文の種別
        ' 3=②見出し                4=③主文
        ' 5=④注意・警戒情報        6=⑩洪水予測の水位状況
...
Sub 主文作成1()
    ...
    If Pattan_Now <> 4 Then
        If Pattan_Now <> 14 Then
            m3 = m3 & vbLf & "　　" & Messag(Pattan_Now).Patn(11)
        End If
        ...
        End If
        If Pattan_Now <> 14 Then
            m3 = m3 & mw & vbLf & "　　" & Messag(Pattan_Now).Patn(13)
            m3 = m3 & "と見込まれます。"
...
Debug.Print "　　見出し(" & Messag(Pattan_Now).Patn(3) & ")"
Debug.Print " "
Debug.Print 主文1
Debug.Print 主文2
...
'            m4 = vbLf & "　　水場川が水位観測所［清須市］で、"
        End If
        If Pattan_Now = 1 Then
            mw = Format(水位.hy2, "#0.0") & "0m 程度"
            水位レベル_Check 水位.hy2 + 0.04, m
        Else
            mw = Format(水位.hy3 + 0.04, "#0.0") & "0m 程度"
            水位レベル_Check 水位.hy3 + 0.04, m
        End If
        m4 = m4 & mw & m
    Else
    ...
    m5 = m5 & vbLf & "　　■レベル２　はん濫注意水位（警戒水位）超過"
...
Debug.Print 主文1
Debug.Print 主文2
Debug.Print "Pattan_Now=" & str(Pattan_Now)


LOG_Out "　　見出し(" & Messag(Pattan_Now).Patn(3) & ")"
LOG_Out " "
LOG_Out 主文1
LOG_Out 主文2
LOG_Out "Pattan_Now=" & str(Pattan_Now)
